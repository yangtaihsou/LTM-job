package io.bitjob.schedule.base.util;
...
public class WorkerUtil {
     ...
     * @throws Exception
     */
    public static boolean isCMPay(String info) throws Exception {
        Map<String, String> map = convertToMap(info);
        String platId = map.get("platId");
        String discount = map.get("discount");
        boolean result = false;
        if (Constants.NEW_PAYMENT_PLAT_CMPAY.equals(platId) && discount != null && !"".equals(discount) && Double.valueOf(discount) > 0) {
            result = true;
        }
        return result;
    }
